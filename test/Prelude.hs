...
import Control.Monad.Error.Lens as X (throwing, throwing_)
...
import System.IO as X (IO, hPrint, hPutStrLn, stderr, stdout)
import Text.Read as X (Read (..), readMaybe)

show :: (BasePrelude.Show a) => a -> Text
show = strToText . BasePrelude.show
